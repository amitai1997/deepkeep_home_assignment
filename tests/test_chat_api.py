...
class TestChatAPI:
    ...
    def test_chat_endpoint_success(self):
        ...
        with patch('src.api.chat.get_moderation_service') as mock_moderation, \
            ...
            mock_moderation_instance.process_message.return_value = (False, False)  # No violation, not blocked
            mock_moderation.return_value = mock_moderation_instance
            
            mock_openai_instance = Mock()
            mock_openai_instance.chat_completion = AsyncMock(return_value=expected_response)
            ...
            mock_moderation_instance.process_message.assert_called_once_with(message, user_id)
    ...
    def test_chat_endpoint_user_blocked(self):
        ...
        with patch('src.api.chat.get_moderation_service') as mock_moderation:
            mock_moderation_instance = Mock()
            mock_moderation_instance.process_message.return_value = (False, True)  # Not violation this time, but blocked
            mock_moderation.return_value = mock_moderation_instance
    ...
    def test_chat_endpoint_with_violation_but_not_blocked(self):
        ...
        with patch('src.api.chat.get_moderation_service') as mock_moderation, \
            ...
            # Setup mocks
            mock_moderation_instance = Mock()
            mock_moderation_instance.process_message.return_value = (True, False)  # Violation but not blocked
            mock_moderation.return_value = mock_moderation_instance
    ...
    def test_chat_endpoint_openai_error(self):
        ...
        with patch('src.api.chat.get_moderation_service') as mock_moderation, \
            ...
            mock_openai_instance = Mock()
            mock_openai_instance.chat_completion = AsyncMock(side_effect=httpx.HTTPError("API Error"))
            mock_openai.return_value = mock_openai_instance
    ...
    def test_chat_endpoint_openai_invalid_response(self):
        ...
        with patch('src.api.chat.get_moderation_service') as mock_moderation, \
            ...
            mock_openai_instance = Mock()
            mock_openai_instance.chat_completion = AsyncMock(side_effect=ValueError("Invalid response"))
            mock_openai.return_value = mock_openai_instance
